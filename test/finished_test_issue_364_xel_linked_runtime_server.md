...
### 确认1（非缺陷）：传统切服下两服共用同一物理路径，本就应各自成键

- [x] 已确认：**按预期行为保留，不修**
- **现象**：传统切服时物理安装路径 `E:\Hypergryph Launcher\games\Arknights Game\Arknights.exe`
  会同时写入 `GamePathBILI`（运行中客户端识别，渠道文件判定为 B服）与 `GamePathCN`（目录扫描，
  该路径本身是官服安装布局）。
- **结论**：这不是冗余。传统切服下两个服**确实就是同一个路径**，两个键各记一份才是如实反映；
  若为此清掉其中一个，用户反而会以为"另一个服不见了"。随游戏自启任务的重复触发器同理无害。
- **说明**：该行为在本次改动前即存在，本次维持不变。
