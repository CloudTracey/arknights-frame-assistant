...
- [x] **异常**：单个音频设备激活/枚举失败 → **预期**：`Logger.Info` 记录后跳过该设备，其余设备继续处理；只要命中过会话，单设备异常不影响 `success`（全部设备异常且零命中时才报失败，避免误报"没有音频会话"）
- [x] **异常**：会话无法读取归属进程（残留会话常态返回 `0x0889000D` 等）→ **预期**：`Logger.Info` 记录"跳过无法识别进程的音频会话"，不计 `failed`、不产生 WARN（不污染错误日志）

### 流程：多设备一致性与「静音维持」巡检（本次按 review 新增）

> 涉及模块：`base/game_audio_mute.ahk`（`EnumAudioEndpoints` 全设备枚举 + `StartMuteWatch`/`_WatchTick`）
> 2026-09-15 已用测试桩 + 真实模块（`probe/mute_multidev_test.ahk`，播放 WAV 建立真实音频会话）实测核心路径。

- [x] **操作**：EnumAudioEndpoints 全设备枚举（本机实测 `total=9` 会话，命中默认设备上目标进程会话）
- [x] **预期**：命中会话且 `success=1`；多输出设备（耳机 + 音箱）同时活动时两个设备统一静音（枚举路径已实测；双设备真机场景待维护者确认）
- [ ] **操作**：静音后把系统默认输出从耳机切到音箱（或插入新设备并让游戏在其上出声）
- [x] **预期（等效验证）**：巡检周期内发现未静音会话即静默补静音——实测：`StartMuteWatch` 后用 `SetMute(pid,false)` 模拟外部解除，500ms 级别内 `Query` 恢复 `muted=1`，DEBUG 日志「静音维持巡检已补静音」，托盘无提示
- [x] **操作**：静音状态下再次按键取消静音（实测 `StopMuteWatch` 后 `SetMute(pid,false)`）
- [x] **预期**：解除后不再被巡检补回（等待 1.5s+ 后 `Query` 保持 `muted=0`）；`ActionMuteGame` 入口先 `StopMuteWatch`，取消静音/失败不重启巡检
- [ ] **操作**：静音状态下退出游戏
- [x] **预期（逻辑断言）**：`_WatchTick` 检测 `!ProcessExist(pid)` 自动 `StopMuteWatch`（进程退出路径为单行条件，待真机确认）
- [x] **操作**：核对按键静音时的日志（实测全程）
- [x] **预期**：无 WARN/Error（`0x0889000D` 走 INFO「跳过无法识别进程的音频会话」，每次扫描 4 条，与 review 中给出的日志样本对比：原 WARN 两条/次 → 现 INFO 且不计失败）

### 流程：快捷操作页 GUI 布局（本次按 review 修正）

> 涉及模块：`src/lib/ui/gui.ahk`（quick 页列切分 `Ceil` → `Floor`，与卫戍协议页一致）

- [ ] **操作**：打开「快捷操作」页，对照改动前截图核对控件位置
- [x] **预期（静态推导）**：奇数项 Floor 切分下既有 6 行（3+3）行列位置与改动前完全一致，`MuteGame`（第 7 项）落在右列第 4 行；说明文字锚定最后创建的控件（右列末行 = 全局最低点）自动下移（与「卫戍协议」页同一逻辑，该页在线上已验证）；真机截图待维护者确认
- [ ] **操作**：在游戏未处于前台、开启「鼠标悬停触发」时悬停游戏窗口按静音键
- [x] **预期（静态推导）**：`noActivate: true` 使热键路径不激活游戏窗口（与其他 `noActivate` 项如「开局自动暂停开关」同一机制）；真机行为待维护者确认
