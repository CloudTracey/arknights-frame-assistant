...
4. `file_extractor.ahk`（文件提取模块，管理编译时嵌入资源的运行时提取——logo.png、3 张 TakeOverButton 图片及 9 张关卡检测投票模板）
...
13. `level_detector.ahk` — 在 `hotkey_actions.ahk` 之后加载，启动 200ms 投票定时器维护关卡状态 `State.InLevel`
...
| `file_extractor.ahk` | 管理编译时 `FileInstall` 嵌入资源的运行时提取。`EnsureExtracted()` 将 `logo.ico`（含大小校验防旧版残留）、三张 `TakeOverButton_*.png`（代理作战按钮图像）和 20 张关卡检测投票模板（6 个对象：费用图标 2 变体/退出按钮 7 变体/关卡内文本 2 变体/蓝堡 2 变体/敌人数 2 变体/暂停按钮 5 变体）统一提取到 `%AppData%\ArknightsFrameAssistant\PC\resources\` |
...
| `level_detector.ahk` | 关卡检测投票状态机（`LevelDetector` 类）。每秒轮询对 6 个关卡内专属对象（费用图标 2 变体/退出按钮 7 变体/关卡内文本 2 变体/蓝堡 2 变体/敌人数 2 变体/暂停按钮 5 变体）做 ImageSearch 小模板匹配，命中 ≥2 个 → `State.InLevel=true`，<2 → `false`。维护守卫判定依据 |
...
- **自动开局暂停流程**（v1.5.3+）：三阶段状态机 — 全屏黑屏检测（`BlackScreenPoints()`）→ Loading 扫描线识别（`LoadingPosition()`，排除红/蓝进关场景）→ 暂停按钮颜色确认后发送 ESC 暂停；暂停后再用代理作战按钮图像检测确认，避免重复暂停。8 秒超时自动取消（`StopSearchLoading()`）。定时器频率随状态动态调整（400ms → 黑屏后 200ms → 超时恢复 400ms）。具体采样点/色值见 `game_monitor.ahk`。
...
- **常规作战关卡守卫与按键透传**（v1.5.12+，hotkey_actions.ahk + hotkey_control.ahk）：常规作战 14 个功能统一经 `GuardInLevel(actionName, ThisHotkey)` 守卫——关卡内放行、关卡外拦截。拦截时由 `KeyForward` 类透传原键：`ForwardOriginalKey()` 对拦截键补发 key down 并记录 `InterceptedKeys` 标志（带 `~` 前缀的键本已透传不补发；Up 型热键只补发 key up；滚轮直接发完整事件），`ActionUpForward()` 为 Up 变体统一回调，松开时按标志补发 key up（事件驱动无阻塞、多键并发正确）。`hotkey_control.ahk` 的 `_RegisterOne()` 为守卫拦截键（非滚轮）注册 `X Up` 变体——类静态方法引用需 `KeyForward.ActionUpForward.Bind(KeyForward)`（直接传引用回调验证失败），`DisableGroup()` 按同规则条件化注销。`GuardInLevel` 判定依据为 `State.InLevel`（LevelDetector 投票状态机维护），无像素检测、无 DPI 切换。拦截日志用 Info 级别（预期行为，不占 critical 轨）。像素检测位置函数统一用 `SafeWinGetClientPos(&ww, &wh)` 获取窗口尺寸——游戏窗口关闭时返回 `false` 而非抛 `TargetError`（`game_monitor.ahk`/`hotkey_actions.ahk` 共用，新增位置函数时遵循此模式）。
- **关卡检测与守卫判定**（level_detector.ahk + hotkey_actions.ahk）：关卡检测改为 `LevelDetector` 投票状态机——每秒对 6 个关卡内专属对象（费用图标 2 变体/退出按钮 7 变体/关卡内文本 2 变体/蓝堡 2 变体/敌人数 2 变体/暂停按钮 5 变体）做 ImageSearch 小模板匹配，命中 ≥2 个置位 `State.InLevel`、<2 复位。`GuardInLevel` 读 `State.InLevel` 判定（无瞬时像素检测），替代旧 `IsInLevel()` 放弃按钮颜色检测（灰色 `0x8c8c8c`/生息演算黄 `0xd8d769`，容差 10）与 `PauseButtonWhite()` 右上暂停按钮白色组合判定。`ActionCeaseOperations`（放弃行动）只发 `battleLeftPopup`、`ActionBack`（返回上级菜单）只发 ESC——两者功能分离于 v1.6.1。
...
- **FileInstall 嵌入资源**：编译时通过 `FileInstall` 将 `logo.ico`、`resources/images/TakeOverButton_*.png`（三张代理作战按钮截图）和 20 张关卡检测投票模板（`CostIcon_1/2`/`ExitLevelButton_1~7`/`TextInLevel_1/2`/`BlueCastle_1/2`/`EnemyCount_1/2`/`PauseButton_1~5`）打包进 exe，运行时由 `FileExtractor.EnsureExtracted()` **统一提取到 `%AppData%\ArknightsFrameAssistant\PC\resources\` 子目录**（避免散落根目录）。`logo.ico` 使用文件大小校验（`FileGetSize`）判断是否需要重新提取以防止旧版本残留。新增需要嵌入的资源时遵循此模式，在 `FileExtractor` 类中添加路径和提取逻辑。
