...
13. `level_detector.ahk` — 在 `hotkey_actions.ahk` 之后加载，启动 1s 投票定时器维护关卡状态 `State.InLevel`
...
| `level_detector.ahk` | 关卡检测投票状态机（`LevelDetector` 类）。每秒轮询对 3 个关卡内专属对象（关卡内文本/退出按钮/暂停按钮）做 PixelSearch 颜色检测（区域用相对比例定位，低分辨率时文本容差放宽到 20），命中 ≥2 个 → `State.InLevel=true`，<2 → `false`。维护守卫判定依据；守卫关闭（`InLevelGuard=0`）时停止轮询并强制 `InLevel=true` |
...
- **常规作战关卡守卫与按键透传**（v1.5.12+，hotkey_actions.ahk + hotkey_control.ahk）：常规作战 14 个功能统一经 `GuardInLevel(actionName, ThisHotkey)` 守卫——关卡内放行、关卡外拦截。拦截时由 `KeyForward` 类透传原键：`ForwardOriginalKey()` 对拦截键补发 key down 并记录 `InterceptedKeys` 标志（带 `~` 前缀的键本已透传不补发；Up 型热键只补发 key up；滚轮直接发完整事件），`ActionUpForward()` 为 Up 变体统一回调。键名经 `GetKeyName` + 小写规范化，且 `InterceptedKeys` 关闭大小写敏感，以匹配 AHK 热键“大小写不敏感、永久保留首次创建拼写”的语义，修复同一物理键以 `a/A` 不同拼写注册时漏发 Up；长按自动重复时不重复补发 Down。`hotkey_control.ahk` 的 `_RegisterOne()` 为守卫拦截键（非滚轮）注册 `X Up` 变体——类静态方法引用需 `KeyForward.ActionUpForward.Bind(KeyForward)`（直接传引用回调验证失败），`DisableGroup()` 按同规则条件化注销。`GuardInLevel` 判定依据为 `State.InLevel`（LevelDetector 投票状态机维护），无像素检测、无 DPI 切换。拦截日志用 Info 级别（预期行为，不占 critical 轨）。像素检测位置函数统一用 `SafeWinGetClientPos(&ww, &wh)` 获取窗口尺寸——游戏窗口关闭时返回 `false` 而非抛 `TargetError`（`game_monitor.ahk`/`hotkey_actions.ahk` 共用，新增位置函数时遵循此模式）。
- **关卡检测与守卫判定**（level_detector.ahk + hotkey_actions.ahk）：关卡检测改为 `LevelDetector` 投票状态机——每秒对 3 个关卡内专属对象（关卡内文本/退出按钮/暂停按钮）做 PixelSearch 颜色检测（区域用相对比例定位，低分辨率时文本容差放宽到 20），命中 ≥2 个置位 `State.InLevel`、<2 复位。`GuardInLevel` 读 `State.InLevel` 判定（无瞬时像素检测），替代旧 `IsInLevel()` 放弃按钮颜色检测（灰色 `0x8c8c8c`/生息演算黄 `0xd8d769`，容差 10）与 `PauseButtonWhite()` 右上暂停按钮白色组合判定。`ActionCeaseOperations`（放弃行动）只发 `battleLeftPopup`、`ActionBack`（返回上级菜单）只发 ESC——两者功能分离于 v1.6.1；`BackCeaseOperations`（Important 配置项，默认关闭）开启后 `ActionBack` 在 ESC 后补发 `battleLeftPopup`，还原旧版"放弃行动"行为。`InLevelGuard`（Important 配置项，默认开启）控制 `GuardInLevel` 守卫开关——关闭后 `LevelDetector` 停止轮询并强制 `State.InLevel=true`，守卫直接放行（零 I/O）；开启后恢复每秒轮询。
