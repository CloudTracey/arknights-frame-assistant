...
4. `file_extractor.ahk`（文件提取模块，管理编译时嵌入资源的运行时提取——logo.png 及 3 张 TakeOverButton 图片）
...
13. `game_monitor.ahk` — 最后加载，启动 800ms 定时器监控游戏进程（含自动退出 + 自动开局暂停检测）
...
| `file_extractor.ahk` | 管理编译时 `FileInstall` 嵌入资源的运行时提取。`EnsureExtracted()` 将 `logo.png`（含大小校验防旧版残留）和三张 `TakeOverButton_*.png`（代理作战按钮图像，用于开局暂停后检测代理作战状态）提取到 `%AppData%\ArknightsFrameAssistant\PC\` |
...
| `hotkey_actions.ahk` | 热键触发后的具体功能实现（`Action*` 函数）。内部 `#Include ./touch_injection.ahk`。所有游戏按键通过 `GameKeys.SendDown`/`SendUp`/`Tap` 发送，不再硬编码。常规作战 14 个功能带关卡守卫（`GuardInLevel`），拦截时经 `KeyForward` 透传原键 |
...
| `game_monitor.ahk` | 三合一游戏状态监控：(1) **自动退出**：游戏进程退出时自动退出 AFA；(2) **自动开局暂停**：通过 17 点全屏黑屏检测 → 三条扫描线 Loading 识别 → 暂停按钮颜色识别的三阶段状态机，在进关卡时自动暂停；(3) 定时器频率随状态动态调整（800ms → 黑屏后 300ms → 8 秒超时恢复 800ms）。包含 `LoadingPosition()`、`BlackScreenPoints()`、`StopSearchLoading()` 三个辅助函数 |

### 关键设计

- **Constants 类**：常量定义。`Delay30`~`Delay240` 是各帧率对应的延迟毫秒值（取 `ceil(1000/fps)`，例外：`Delay144=8` 多 1ms 余量），`State.CurrentDelay` 依此计算。`FrameOptions` 定义下拉框选项数组，`FrameTextToOldIndex`/`FrameOldIndexToText` 用于 Frame155 双写转换。`KeyNames` 是一个 `Map(keyName, displayName)`，定义所有热键 key 到 GUI 中文显示名的映射——新增热键功能时**必须**在此 Map 中同步添加条目。`CustomNames` 对应自定义设置的显示名，新增自定义配置项时也需同步添加，否则设置无法保存。
...
- **State 类**：运行时状态。`CurrentDelay` 根据游戏帧率设置计算（30/60/90/120/144/165/180/240+ 帧对应不同的 ms 延迟值），引用 `Constants.Delay*` 常量。`GameHasStarted` 跟踪游戏是否曾运行过（用于自动退出判断）。`ReadyForPause` 和 `BlackScreenDetected` 是自动开局暂停状态机的两个标志位。
...
- **自动开局暂停流程**（v1.5.3+）：三阶段状态机 — 全屏黑屏检测（`BlackScreenPoints()`）→ Loading 扫描线识别（`LoadingPosition()`，排除红/蓝进关场景）→ 暂停按钮颜色确认后发送 ESC 暂停；暂停后再用代理作战按钮图像检测确认，避免重复暂停。8 秒超时自动取消（`StopSearchLoading()`）。定时器频率随状态动态调整（800ms → 黑屏后 300ms → 超时恢复 800ms）。具体采样点/色值见 `game_monitor.ahk`。
- **热键注册与拦截**：用 `HotIfWinActive("ahk_exe Arknights.exe")` 限制热键仅对明日方舟生效。通过 `GameKeys.GetInterceptPattern()` 动态生成拦截正则——从注册表读取所有游戏按键 + `Escape|RButton|MButton`。AFA 热键绑定的按键若匹配拦截正则，不加 `~` 前缀（阻止原键传递到游戏），否则加 `~` 前缀（透传）。用户自定义游戏按键后，轮询检测到注册表变更自动重建热键，拦截列表随之更新。
- **常规作战关卡守卫与按键透传**（v1.5.12+，hotkey_actions.ahk + hotkey_control.ahk）：常规作战 14 个功能统一经 `GuardInLevel(actionName, ThisHotkey)` 守卫——关卡内放行、关卡外拦截。拦截时由 `KeyForward` 类透传原键：`ForwardOriginalKey()` 对拦截键补发 key down 并记录 `InterceptedKeys` 标志（带 `~` 前缀的键本已透传不补发；Up 型热键只补发 key up；滚轮直接发完整事件），`ActionUpForward()` 为 Up 变体统一回调，松开时按标志补发 key up（事件驱动无阻塞、多键并发正确）。`hotkey_control.ahk` 的 `_RegisterOne()` 为守卫拦截键（非滚轮）注册 `X Up` 变体——类静态方法引用需 `KeyForward.ActionUpForward.Bind(KeyForward)`（直接传引用回调验证失败），`DisableGroup()` 按同规则条件化注销。`GuardInLevel` DPI 上下文自包含（检测期间临时 per-monitor、返回前恢复，调用方无需传 oldCtx）。拦截日志用 Info 级别（预期行为，不占 critical 轨）。像素检测位置函数统一用 `SafeWinGetClientPos(&ww, &wh)` 获取窗口尺寸——游戏窗口关闭时返回 `false` 而非抛 `TargetError`（`game_monitor.ahk`/`hotkey_actions.ahk` 共用，新增位置函数时遵循此模式）。
- **放弃作战按钮识别与守卫组合判定**（v1.6.1+，hotkey_actions.ahk）：`IsInLevel()` 只判定左上放弃按钮区域（灰色 `0x8c8c8c` / 生息演算黄色 `0xd8d769`，容差 10），供 `ActionBeginPause` 二次确认与守卫共用。v1.6.2 起守卫为组合判定 `IsInLevel() AND PauseButtonWhite()`——后者检测右上暂停按钮区域白色（`0xffffff`，容差 20），修复部分设备上放弃按钮灰色偏色导致的作战按键完全失效（曾单靠提高容差到 20 不稳，回退 10 后以组合判定保持稳定）。`ActionCeaseOperations`（放弃行动）只发 `battleLeftPopup`、`ActionBack`（返回上级菜单）只发 ESC——两者功能分离于 v1.6.1（此前共通）。
- **GameKeys 类**（v1.5.10+）：负责游戏按键的动态识别。核心流程：`Init()` 首次读取注册表并启动 10s 定时器 → `_OnPoll()` 定时比对 hex，有变更则重新解析 JSON → 更新 `_Bindings` → 调用 `HotkeyController.EnableByTab()` 重建热键。`SendDown`/`SendUp`/`Tap` 三个方法封装了查表+Send 逻辑，接受注册表中的 function 名（如 `"releaseSkill"`），内部转换为用户实际绑定的 AHK 键名。注册表键名前缀匹配 `KEYBOARD_SETTING_V` 应对游戏版本更新。ESC 和 LButton 不经过 GameKeys（不可重新绑定的系统级按键）。
- **更新渠道**：`UpdateChannel` 设置为 1（正式版）或 2（测试版），版本检查器据此选择检查 stable releases 还是包含 pre-release。GUI 通过下拉框切换，默认正式版。
- **双源更新与自动降级**（v1.5.6+）：更新系统支持 GitHub API 和国内源（腾讯云 COS+CDN）两源，`UpdateSource` 选首选源，失败自动降级备选源（`token_invalid`/`rate_limited` 静默降级）。国内源用 CDN 静态 `version.json`（`version`/`downloadUrl`/`releases`），`releases` 格式与 GitHub API 一致，复用 changelog 缓存。发布时 Action（`.github/workflows/release-sync.yml`）自动同步 exe 和 version.json 到 COS。
...
- **"其他设置"页面结构**（v1.5.5+）：左侧 Text 导航项（`NavItems` 数组）+ 右侧分类内容叠加。四个分类（`LaunchControls`/`UpdateControls`/`CustomControls`/`AboutControls`）控件在相同位置叠加，通过 `_SwitchOtherCategory` 切换 Visible。`OtherCategories` Map（分类名→[控件组, 导航索引]）和 `_HideOtherCategories()` 统一管理分类切换，新增分类只需在 Map 中加一行。导航切换有 `force` 参数：标签页切换时 `force=true` 强制显示，导航项点击时不传 force 以守卫重复点击。关于页是纯展示页，不含可修改设置，切换到时禁用保存/应用按钮。
...
- **FileInstall 嵌入资源**：编译时通过 `FileInstall` 将 `logo.png` 和 `resources/images/TakeOverButton_*.png`（三张代理作战按钮截图）打包进 exe，运行时由 `FileExtractor.EnsureExtracted()` 提取到 `%AppData%\ArknightsFrameAssistant\PC\`。`logo.png` 使用文件大小校验（`FileGetSize`）判断是否需要重新提取以防止旧版本残留。新增需要嵌入的资源时遵循此模式，在 `FileExtractor` 类中添加路径和提取逻辑。
- **GitHub Action 发布同步**（v1.5.6+）：`.github/workflows/release-sync.yml` 监听 Release 发布事件，将 `AFA.exe` 和 `version.json` 上传到 COS 并刷新 CDN。`.github/scripts/build_version_json.py` 构建含全量 releases 历史的 `version.json`，处理首次初始化（COS 上无文件时自动创建），支持 stable/beta 双通道独立 version.json。Action 需要 5 个 GitHub Secrets（`COS_SECRET_ID`/`COS_SECRET_KEY`/`COS_BUCKET`/`COS_REGION`/`CDN_DOMAIN`）。`release-sync.yml` 不在 `.gitignore` 中，会被 git 跟踪。
- **GUI 控件统一管理**：对于批量重复的控件组（如过帧延迟字段、导航分类），优先使用列表/Map 集中定义再循环遍历（如 `FrameSkipDelayKeys`、`OtherCategories`），避免单个 try 块的 OR 链，便于扩展。
- **顶部标签页管理器**（v1.5.12+，gui.ahk）：`TabItems` 数组描述四个标签（`keyBind`/`quick`/`strongHoldProtocol`/`other`），含 `CanHide` 标志（`other` 为 false 不可隐藏）。`TabOrder`/`HiddenTabs` 两个 Important 配置项存顺序与隐藏列表，通过两个 **Hidden Edit 表单变量**（`vTabOrder`/`vHiddenTabs`）与 `MainGui["TabOrder"]` 交互——它们必须放在布局链之外（如 `sepCustom` 之前），否则会破坏自定义页左列 `y+10` 相对定位。`AppliedTabSettings` 保存已应用的 {Order, Visibility} 快照，`IsTabVisible()` 优先读快照、`tabItem.Visible` 是工作态——统计当前可见性需直接遍历 `tabItem.Visible`（用 `IsTabVisible` 会读到未应用的状态）。眼睛图标统一 `U+E890`（睁眼）+ 颜色区分（蓝=显示/灰=隐藏），禁止隐藏最后一个功能标签时弹窗。
...
在 `src/lib/version.ahk` 的 `Version.Number` 中定义，版本检查器通过 GitHub API 或国内源 CDN 对比此值与远程 release tag/version.json。
