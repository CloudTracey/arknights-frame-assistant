...
| `level_detector.ahk` | 关卡检测投票状态机（`LevelDetector` 类）。每秒轮询对 3 个关卡内专属对象（关卡内文本/退出按钮/暂停按钮）做 PixelSearch 颜色检测（区域用相对比例定位，低分辨率时文本容差放宽到 20），命中 ≥2 个 → `State.InLevel=true`，<2 → `false`。维护守卫判定依据 |
