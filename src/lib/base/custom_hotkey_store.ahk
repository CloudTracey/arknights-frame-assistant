...
;   1. 写定式：键序固定（key/name/script/type）、紧凑单行，字符串只转义 \ " 与 \r\n\t；
;   2. 字符集白名单（名称禁引号/反斜杠/控制字符，脚本语法天然不含），读取器可用严格正则完整解析；
...
class CustomHotkeyStore {
    ...
    ; 返回 Array<{Key, Name, Script, Type}>（type 为原始字符串，非法值由 Config 层宽容回退）。
    static Load() {
        if this.File = ""
            this.InitPath()
    ...
    static _Serialize(entries) {
        ...
        for i, entry in entries {
            ...
            body .= '{"key":' this._Quote(entry.Key)
                  ...
                  . ',"script":' this._Quote(entry.Script)
                  . ',"type":"' entry.Type '"}'
        }
        return '{"version":1,"keys":[' body "]}"
    }

    ; JSON 字符串转义：\ " 与 \r \n \t（其余控制字符由上游校验排除，脚本语法天然不含）
    static _Quote(s) {
        s := StrReplace(s, "\", "\\")
        s := StrReplace(s, '"', '\"')
        s := StrReplace(s, "`r", "\r")
    ...
    static _Parse(text) {
        text := Trim(text)
        if !RegExMatch(text, '^\{"version":1,"keys":\[(.*)\]}$', &m)
            return ""
        inner := m[1]
        entries := []
        if inner != "" {
            ...
            for i, part in parts {
                ...
                if !RegExMatch(candidate, '^\{"key":"([^"]*)","name":"([^"]*)","script":"([^"]*)","type":"([^"]*)"\}$', &em)
                    return ""
                entries.Push({
                    Key: this._Unescape(em[1]),
                    Name: this._Unescape(em[2]),
                    Script: this._Unescape(em[3]),
                    Type: em[4]
                })
            }
        }
